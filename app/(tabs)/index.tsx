...
export default function TabOneScreen() {
  ...
  async function postData(data: Body) {
    // Request 
    return fetch('http://xquad3.pythonanywhere.com/api-endpoint', {
      method: 'POST',
      headers: {
  ...
  return (
    <View style={styles.container}>
      <InputValues 
        ...
        onChange={(value: number) => setPumpData({...pumpData, preco_por_litro: value })}/>
      <InputValues 
        text='Quantidade de gasolina que apareceu na bomba:' 
        imagePath='../assets/images/vector_oil.png' 
        style={{width: 27, height: 30}}
        onChange={(value: number) => setPumpData({...pumpData, litros_totais: value })}/>
